...
class NeuralNetworkParameterAnalyzer:
    ...
    def extract_parameters(self, folder_name):
        """Extract noise, fp, cde, cdi, and noise from folder name"""
        pattern = r'batch_hip([\d.]+)_fp([\d.]+)_cde([\d.]+)_cdi([\d.]+)_noise([\d.]+)'
    ...
    def load_all_data(self):
        ...
        if all_data:
            ...
            for param in ['noise', 'fp', 'cde', 'cdi', 'noise']:
                if param in self.data.columns:
                    self.data[param] = pd.to_numeric(self.data[param], errors='coerce')
                    self.data[param].fillna(0, inplace=True)  # Fill NaN with 0 for parameters        
            ...
            if all_individual_data:
                ...
                for overall_metric, individual_metric in metric_mapping.items():
                    if overall_metric in self.metrics and individual_metric in combined_individual.columns:
                        ...
                        std_data = combined_individual.groupby(['noise', 'fp', 'cde', 'cdi'])[individual_metric].std().reset_index()
                        std_data.rename(columns={individual_metric: f'{overall_metric}_std'}, inplace=True)
                        
                        # Merge with main data
                        self.data = pd.merge(self.data, std_data, on=['noise', 'fp', 'cde', 'cdi'], how='left')
            ...
            print(f"Parameters found: noise, fp, cde, cdi")
    ...
    def create_avalanche_scaling_plots(self, save_path=None, fit_noise_range=(0.02, 0.08)):
        """
        Create plots for AVsize, AVduration, and difference vs noise with fitted lines,
        ...
        fit_noise_range : tuple, optional
            (min, max) range of noise values to use for fitting the baseline.
            Default is (0.02, 0.08).
            Example: (0.01, 0.09) to fit only points where 0.01 <= noise <= 0.09
        """
        ...
        # Filter data to only rows that have noise and at least one non-zero AV metric
        mask = self.data['noise'].notna()
        for col in available_av_columns:
            mask = mask & ((self.data[col].notna()) & (self.data[col] != 0))
        
        clean_data = self.data[mask][['noise', 'param_combo', 'fp', 'cde', 'cdi'] + available_av_columns].copy()
        ...
        # Apply noise range filter for fitting if specified
        if fit_noise_range is not None:
            fit_baseline_data = baseline_data[
                (baseline_data['noise'] >= fit_noise_range[0]) & 
                (baseline_data['noise'] <= fit_noise_range[1])
            ]
            print(f"Fitting baseline data only for noise in range [{fit_noise_range[0]}, {fit_noise_range[1]}]")
        ...
        # Create color map for different parameter combinations (matching noise plots exactly)
        ...
        for i, combo in enumerate(sorted(all_unique_combos)):
            if is_baseline(combo):
                ...
                # Use Set1 colormap for more distinct colors (matching noise plots)
        ...
        with PdfPages(save_path) as pdf:
            ...
            fig.suptitle('Avalanche Scaling Analysis vs Noise', fontsize=16, y=0.98)
            ...
            for metric, title, ax in metrics_info:
                ...
                if len(fit_baseline_data) > 0:
                    ...
                    x_fit_data = fit_baseline_data['noise'].values  # No log transform for noise
                    ...
                    if len(x_fit_data) >= 2:
                        ...
                        fit_results[metric] = {
                            ...
                            'noise_range': fit_noise_range if fit_noise_range else (fit_baseline_data['noise'].min(), fit_baseline_data['noise'].max())
                        }
                        
                        # Create fit line that extends beyond all data points
                        all_x = np.log10(clean_data['noise'].values)
                        ...
                        ax.plot(10**x_line, y_line, color='black', linewidth=2.5, linestyle='-', 
                            alpha=0.8, zorder=3,
                            label=f'Baseline fit: y = {slope:.3f}·noise + {intercept:.3f}')
                        
                        # Add shaded region showing the fitting range
                        if fit_noise_range is not None:
                            ax.axvspan(fit_noise_range[0], fit_noise_range[1], alpha=0.1, color='green', 
                                    label='Fitting range', zorder=1)
                        
                        # Add fit statistics to plot
                        if fit_noise_range is not None:
                            range_str = f'noise ∈ [{fit_noise_range[0]:.3f}, {fit_noise_range[1]:.3f}]'
                        else:
                    ...
                    # Plot ALL baseline data points with noise labels
                    if fit_noise_range is not None and len(baseline_data) > len(fit_baseline_data):
                        ...
                        if len(out_of_range) > 0:
                            ax.scatter(out_of_range['noise'], out_of_range[metric], 
                            ...
                            for _, row in out_of_range.iterrows():
                                ax.annotate(f'{row["noise"]:.3f}', 
                                           (row['noise'], row[metric]),
                                           xytext=(2, 2), textcoords='offset points',
                                           fontsize=6, alpha=0.7, color='gray')
                        
                        # Plot points within fitting range in black
                        ax.scatter(fit_baseline_data['noise'], fit_baseline_data[metric], 
                        ...
                        ax.scatter(baseline_data['noise'], baseline_data[metric], 
                                color='black', alpha=0.8, s=120, edgecolor='white', 
                ...
                # Set x-axis to show all noise values
                # Set x-axis to show all noise values
                all_noise = sorted(clean_data['noise'].unique())
                ax.set_xticks(all_noise)
                ax.set_xticklabels([f'{x:.4g}' for x in all_noise])
            ...
            if 'AV_Alpha' in available_av_columns and 'AV_Beta' in available_av_columns:
                ...
                if len(fit_baseline_data) > 0:
                    ...
                    # Plot baseline points with noise labels
                    if fit_noise_range is not None and len(baseline_data) > len(fit_baseline_data):
                        # Plot points outside fitting range in gray
                        out_of_range = baseline_data[
                            (baseline_data['noise'] < fit_noise_range[0]) | 
        ...
        with open(fit_results_path, 'w', encoding='utf-8') as f:
            ...
            if fit_noise_range is not None:
                f.write(f"Fitting range: noise ∈ [{fit_noise_range[0]}, {fit_noise_range[1]}]\n")
            else:
                f.write("Fitting range: All baseline data points\n")
            f.write("\nLinear fits for baseline parameters (fp=cde=cdi=0):\n")
            f.write("metric = slope * log10(noise) + intercept\n\n")
            ...
            if fit_results:
                for metric in available_av_columns:
                    if metric in fit_results:
                        ...
                        f.write(f"  noise range: [{fit_results[metric]['noise_range'][0]:.4f}, {fit_results[metric]['noise_range'][1]:.4f}]\n\n")
                ...
                # Check if alpha increases with noise
                if 'AV_Alpha' in fit_results:
                    alpha_slope = fit_results['AV_Alpha']['slope']
                    if alpha_slope > 0:
                        f.write("- AVsize exponent (α) increases with noise\n")
                        f.write("  → Avalanche size distributions become less heavy-tailed at higher noise\n")
                    else:
                        f.write("- AVsize exponent (α) decreases with noise\n")
                        f.write("  → Avalanche size distributions become more heavy-tailed at higher noise\n")
                
                # Check if beta increases with noise
                if 'AV_Beta' in fit_results:
                    beta_slope = fit_results['AV_Beta']['slope']
                    if beta_slope > 0:
                        f.write("- AVduration exponent (β) increases with noise\n")
                        f.write("  → Avalanche duration distributions become less heavy-tailed at higher noise\n")
                    else:
                        f.write("- AVduration exponent (β) decreases with noise\n")
                        f.write("  → Avalanche duration distributions become more heavy-tailed at higher noise\n")
                ...
                if 'AV_Scaling_Diff' in fit_results:
                    ...
                    if abs(diff_slope) < 0.1:
                        f.write("- Scaling difference remains relatively constant with noise\n")
                        f.write("  → Scaling relation consistency across noise values\n")
                    elif diff_slope > 0:
                        f.write("- Scaling difference increases with noise\n")
                        f.write("  → Deviation from theoretical scaling increases at higher noise\n")
                    else:
                        f.write("- Scaling difference decreases with noise\n")
                        f.write("  → Better agreement with theoretical scaling at higher noise\n")
            else:
    ...
    def create_baseline_comparison(self, save_path=None):
        """Create visualizations showing how noise trends change with different parameters"""
        if save_path is None:
        ...
        with PdfPages(save_path) as pdf:
            # 1. Side-by-side comparison of how each parameter modifies noise trends
            n_metrics = len(self.metrics)
            n_cols = min(3, n_metrics)
            n_rows = (n_metrics + n_cols - 1) // n_cols
            
            fig, axes = plt.subplots(n_rows, n_cols, figsize=(20, 16))
            fig.suptitle('How Parameters Modify noise Trends', fontsize=16)
            ...
            for idx, metric in enumerate(self.metrics):
                ...
                if not baseline.empty:
                    ax.plot(baseline['noise'], baseline[metric], 
                ...
                if not fp_data.empty:
                    for fp_val in sorted(fp_data['fp'].unique())[:3]:  # Top 3 values
                        subset = fp_data[fp_data['fp'] == fp_val].sort_values('noise')
                        ax.plot(subset['noise'], subset[metric], 
                ...
                if not cde_data.empty:
                    for cde_val in sorted(cde_data['cde'].unique())[:3]:
                        subset = cde_data[cde_data['cde'] == cde_val].sort_values('noise')
                        ax.plot(subset['noise'], subset[metric], 
                ...
                # Set x-axis to show all discrete noise values
                unique_noises = sorted(self.data['noise'].unique())
                ax.set_xticks(unique_noises)
                ax.set_xticklabels([f'{x:.3f}' for x in unique_noises])
            ...
            # 2. Slope analysis - how parameters change the noise slope
            fig, axes = plt.subplots(3, 3, figsize=(20, 16))
            fig.suptitle('noise Slope Changes by Parameter Settings', fontsize=16)
            axes_flat = axes.flatten()
            ...
            for idx, metric in enumerate(self.metrics[:9]):  # Max 9 metrics
                ...
                for _, combo in param_combos.iterrows():
                    ...
                    if len(subset) >= 2:  # Need at least 2 points for slope
                        try:
                            ...
                            log_noise = np.log10(subset['noise'])
                            slope, intercept = np.polyfit(log_noise, subset[metric], 1)
                ...
                if slope_data:
                    ...
                    if box_data:
                        ...
                        ax.set_ylabel('Slope (d(metric)/d(log noise))', fontsize=10)
                        ax.set_title(self.metric_names[metric], fontsize=12)
                        ax.grid(True, alpha=0.3)
            ...
            # 3. Parameter interaction effects on noise trends
            fig, axes = plt.subplots(2, 2, figsize=(20, 12))
            fig.suptitle('Parameter Interaction Effects on Key Metrics vs noise', fontsize=16)
            ...
            for idx, metric in enumerate(key_metrics):
                ...
                # Set x-axis to show all discrete noise values
                unique_noises = sorted(pivot.index)
                ax.set_xticks(unique_noises)
                ax.set_xticklabels([f'{x:.3f}' for x in unique_noises])
                plt.setp(ax.xaxis.get_majorticklabels(), rotation=45, ha='right')
    ...
    def create_color_coded_hip_plots(self, save_path=None):
        """Create noise vs metric plots with color coding for different parameter combinations"""
        if save_path is None:
        ...
        with PdfPages(save_path) as pdf:
            ...
            title_ax.axis('off')
            title_ax.text(0.5, 0.5, 'Neural Network Metrics vs Noise - Parameter Effects',
            ...
            for idx in range(n_metrics):
                ...
                if baseline_combo:
                    baseline_data = self.data[self.data['param_combo'] == baseline_combo].sort_values('noise')
                    if not baseline_data.empty:
                        # Plot the line and markers
                        ax.plot(baseline_data['noise'], baseline_data[metric],
                        ...
                        if hasattr(self, 'std_columns') and metric in self.std_columns:
                            ...
                            if yerr_col in baseline_data.columns and baseline_data[yerr_col].notna().any():
                                ...
                                x_values = baseline_data['noise'].values
                ...
                for combo in sorted(unique_combos):
                    ...
                    combo_data = self.data[self.data['param_combo'] == combo].sort_values('noise')
                    if not combo_data.empty:
                        # Plot the line and markers
                        ax.plot(combo_data['noise'], combo_data[metric],
                        ...
                        if hasattr(self, 'std_columns') and metric in self.std_columns:
                            ...
                            if yerr_col in combo_data.columns and combo_data[yerr_col].notna().any():
                                # Create semi-transparent rectangles for error regions
                                x_values = combo_data['noise'].values
                ...
                ax.set_xlabel('noise', fontsize=9, fontweight='semibold', labelpad=2)  # Closer to axis
                ...
                # Set x-axis to show exact noise values without rounding
                unique_noises = sorted(self.data['noise'].unique())
                ax.set_xticks(unique_noises)
                # Format labels to show exact values
                ax.set_xticklabels([f'{x:.4g}' for x in unique_noises])
        ...
        # Continue with Page 2 code...
            ...
            for metric in plot_metrics:
                # Get all data sorted by noise
                sorted_data = self.data.sort_values('noise')

                # Group by noise and calculate mean for each noise value
                grouped = sorted_data.groupby('noise')[metric].mean()
            ...
            ax.set_title('All Metrics Overlaid - Co-occurring Changes Across noise\n(D2 Correlation and Branching Ratios share the same scale)', 
                        fontsize=14, pad=10)
            ax.set_xscale('log')
            ax.grid(True, alpha=0.3)
        ...
        print(f"Color-coded noise analysis saved to {save_path}")
    ...
    def create_correlation_analysis(self, save_path=None):
        ...
        analysis_cols = ['noise', 'fp', 'cde', 'cdi'] + self.metrics
        ...
        with PdfPages(save_path) as pdf:
            ...
            labels = ['noise', 'fp', 'cde', 'cdi'] + [self.metric_names[m] for m in self.metrics]
