...
def run_single_simulation_with_eigenvalues(sim_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    plot_raster_with_activity_from_hdf5(
        filename,
        save_path=os.path.join(run_dir, "raster_with_activity.pdf"),
...
def plot_connection_fraction_with_activity(filename: str, run_dir: str, run_id: str, 
    ...
    with h5py.File(filename, 'r') as f:
        ...
        # Plot activity
        activity_data = f['activity_E'][:]
        # Downsample for plotting
        downsample_factor = 1000
        activity_downsampled = activity_data[::downsample_factor]
        x_activity = np.arange(len(activity_downsampled)) * downsample_factor
        
        ax2.plot(x_activity, activity_downsampled, 'k-', linewidth=0.8, alpha=0.7)
        ...
        ax2.set_xlabel(r'$10^6$ time steps', fontsize=12)
        ...
        ax2.set_xticks(np.arange(0, n_millions + 1) * 1e6)
        ax2.set_xticklabels([str(i) for i in range(n_millions + 1)])
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(run_dir, 'connection_fraction_with_activity.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(PARAM_VALUES) > 1:
            ...
            for param_val in PARAM_VALUES:
                if eigenvalue_results_by_param[param_val]:
                    act_vals = [ea['activity_at_checkpoint'] for ea in eigenvalue_results_by_param[param_val]]
                    print(f"  {PARAM_TO_SWEEP}={param_val}: {np.mean(act_vals):.4f} ± {np.std(act_vals):.4f}")
        ...
        for param_val in PARAM_VALUES:
            ...
            for i in range(N_SEEDS_PER_VALUE):
                ...
                print(f"        ├── raster_zoom_last100k_coarse.png")
                print(f"        ├── raster_with_activity.pdf")
                print(f"        ├── connection_fraction_with_activity.png")
