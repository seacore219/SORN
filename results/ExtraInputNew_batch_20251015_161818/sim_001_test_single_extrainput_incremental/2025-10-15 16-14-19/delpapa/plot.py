...
def plot_results(result_path,result):
    ...
    try:
        # Try first method - get raw data
        pickle_dir = data.c.logfilepath.read()[0]
    except:
        try:
            # Try second method - get object ID as string
            pickle_dir = str(data.c.logfilepath._v_objectid)
        except:
            try:
                # Try third method - read first element directly
                pickle_dir = data.c.logfilepath._g_read()[0]
            except:
                print("Warning: Could not read logfilepath, using default path")
                pickle_dir = "."
    ...
    #if False: #if data.__contains__('Spikes'):
    if data.__contains__('Spikes'):
        ...
        for (i,sp) in enumerate(spikes):
            ...
            if s_train != []:
				...
				#hold('on')
                plt.vlines(s_train, i + 0.5, i + 1.5, colors='r', linewidth=1.0)
    ...
    if data.__contains__('SpikesInh'):
        ...
        for (i,sp) in enumerate(spikes):
            s_train = where(sp == 1)[0]
            if s_train != []:
				plt.vlines(s_train, i + 0.5, i + 1.5)
				#hold('on')
...
def plot_results_perturbation(result_path,result):
    ...
    try:
        ...
        try:
            # Alternative: get raw data
            pickle_dir = data.c.logfilepath._g_read()[0]
        except:
            print("Warning: Could not read logfilepath, using default")
            pickle_dir = "."

    print("Debug - pickle_dir type:", type(pickle_dir))  # Add this debug line
        ...
        for (i,sp) in enumerate(only_non_pert):
            ...
            if s_train != []:
                if i == 1: # small change to add the label - TODO looks terrible, change this ASAP
                    plt.vlines(s_train, i + 0.5, i + 1.5, 'b', label = 'Without perturbation only')
                else:
                    plt.vlines(s_train, i + 0.5, i + 1.5, 'b')
                #hold('on')
        for (i,sp) in enumerate(only_pert):
            s_train = where(sp == 1)[0]
            if s_train != []:
                if i == 1: # small change to add the label - TODO looks terrible, change this ASAP
                    plt.vlines(s_train, i + 0.5, i + 1.5, 'r', label = 'With perturbation only')
                else:
                    plt.vlines(s_train, i + 0.5, i + 1.5, 'r')
                #hold('on')
        ylabel('Excitatory Neuron')
        legend(loc='best')
        xlim([0,1000])
